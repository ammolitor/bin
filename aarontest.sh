...
CREDS=$(aws sts get-session-token --serial-number arn:aws:iam::953558963498:mfa/aarontest --token-code ${MFACODE} --profile aarontest | jq -r ' . | .Credentials | "\(.AccessKeyId) \(.SecretAccessKey) \(.SessionToken)"')
